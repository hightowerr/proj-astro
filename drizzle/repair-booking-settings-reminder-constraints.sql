...
WHERE cardinality(reminder_timings) = 0;
...
ALTER TABLE booking_settings
  ADD CONSTRAINT booking_settings_reminder_timings_max
  CHECK (cardinality(reminder_timings) <= 3);

ALTER TABLE booking_settings
  ADD CONSTRAINT booking_settings_reminder_timings_min
  CHECK (cardinality(reminder_timings) >= 1);
